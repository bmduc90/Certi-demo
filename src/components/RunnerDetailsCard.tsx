...
export const RunnerDetailsCard: React.FC<RunnerDetailsCardProps> = ({ runner }) => {
  ...
  return (
    <div className="w-full bg-white border border-stone-200/90 rounded-2xl p-4 sm:p-6 shadow-xs text-stone-800 space-y-4" id="runner-details-card">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs px-2 py-0.5 rounded-md bg-stone-100 text-stone-700 font-neue-plak font-bold border border-stone-200">
                BIB: {runner.bib}
              </span>
            </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-7 gap-2.5">
        ...
        <div className="bg-stone-50/70 p-3 rounded-xl border border-stone-200/70 flex flex-col justify-between">
          ...
          <span className="text-base sm:text-lg font-bold text-stone-900 mt-1 font-neue-plak">
            {runner.bib}
          </span>
        </div>
        ...
        <div className="bg-teal-50/60 p-3 rounded-xl border border-teal-200/70 flex flex-col justify-between">
          ...
          <span className="text-base sm:text-lg font-extrabold text-teal-900 mt-1 font-neue-plak">
            {runner.chipTime}
          </span>
        </div>
        ...
        <div className="bg-stone-50/70 p-3 rounded-xl border border-stone-200/70 flex flex-col justify-between">
          ...
          <span className="text-base sm:text-lg font-bold text-stone-800 mt-1 font-neue-plak">
            {runner.gunTime}
          </span>
        </div>
        ...
        <div className="flex flex-wrap items-center justify-between gap-2 border-b border-stone-800 pb-2.5">
          ...
          <div className="flex items-center gap-3 text-xs">
            <span className="text-stone-400">
              Avg Pace:{' '}
              <strong className="text-amber-300 font-neue-plak">{splits.avgPace}</strong>
            </span>
            <span className="text-stone-500">•</span>
            <span className="text-stone-400">
              GunTime: <strong className="text-stone-200 font-neue-plak">{splits.gunTime}</strong>
            </span>
            <span className="text-stone-500">•</span>
            <span className="text-stone-400">
              ChipTime: <strong className="text-teal-300 font-neue-plak">{splits.chipTime}</strong>
            </span>
          </div>
        </div>
          ...
          {splits.checkpoints.map((cp, idx) => (
            ...
            >
              <div className="flex items-center justify-between">
                ...
                {idx > 0 && (
                  <span className="text-[10px] px-1.5 py-0.2 rounded bg-teal-400/10 text-teal-300 border border-teal-400/20 font-neue-plak">
                    {cp.pace}/km
                  </span>
                )}
              </div>
              ...
              <div className="mt-1 pt-1 border-t border-stone-700/50 text-[10px] text-stone-400 flex items-center justify-between">
                ...
                <span className="font-semibold text-stone-300 font-neue-plak">
